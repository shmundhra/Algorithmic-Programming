...
{   
    ...
    {   
        ll n ; cin >> n ; inarr(n);
        vec<vll> hash(1e6+1) ; lp(i,n) hash[v[i]].eb(i);

        vll count(n,0);
        lp(i,n)
        {   
            ll val = v[i];
            if (hash[val].back() != i) continue;
            for(ll j = val; j <= 1e6; j+= val)
            {
                count[i] += lower_bound(all(hash[j]), i) - hash[j].begin();  
            }
        }
        cout << *maxof((count)) << endl;
    }
    return 0 ;  
}
