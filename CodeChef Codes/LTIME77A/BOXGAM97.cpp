//  https://www.codechef.com/LTIME77A/problems/BOXGAM97
...
signed main ()
{
    ...
    {   
        ...
        if (k%2 is 1 and p is 0) {
            prv(*maxof(v));
            continue;
        }   

        if (k%2 is 1 and p is 1) {
            prv(*minof(v));
            continue;
        }
        ...
        if (p is 0)
        {
            v.insert(v.begin(), 1e10);
            v.pb(1e10);

            ll ans = 0;
            for(int i = 1; i <=n ; i++)
            {
                ans = mx(ans, mn(v[i-1], v[i+1]));
            }
            prv(ans); 
            continue;
        }
