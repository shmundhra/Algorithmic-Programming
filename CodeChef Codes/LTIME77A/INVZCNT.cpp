//  https://www.codechef.com/LTIME77A/problems/INVZCNT
...
void solve(int b)
{
	...
	while(!mast.empty())
	{
		...
		// show zeros sp;
		// show ones sp;
		if (sz(zeros) > 1) rest.pb(zeros);
		if (sz(ones) > 1 ) rest.pb(ones);
	}
	while(!rest.empty())
	{
		mast.pb(rest.front());
...
signed main ()
{
    ...
    tc(t) 
    {	
    	ll n, q ; cin >> n >> q ; invec(v,n);
    	zero.assign(32, 0);
    	one.assign(32, 0);

		int start = log2(*maxof(v)) + 1;

    	mast.pb(v);

    	for(int b = start; b >= 0; b--) solve(b);
