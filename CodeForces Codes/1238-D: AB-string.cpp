...
signed main ()
{
    ...
    lp(i,sz(s)) {
        if (s[i] is 'A') {
            if (0 <= i and i < n) A[i].F = i-prev;
            if (0 <= prev and prev < n) A[prev].S = i-prev;
            prev = i;
        }
    }
    s.pop_back();
    ...
    lp(i,sz(s)) {
        if (s[i] is 'B') {
            if (0 <= i and i < n) B[i].F = i-prev;
            if (0 <= prev and prev < n) B[prev].S = i-prev;
            prev = i;
        }
    }
    s.pop_back();
    ...
    lp(i,n) {
        if (s[i] is 'A') {
            count -= A[i].F + max(A[i].S-2, 0); 
        }
        else {
            count -= B[i].F + max(B[i].S-2, 0); 
        }
    }
    cout << count << endl;
