...
int check(set<int>& s1, set<int>& s2, set<int>& s3)
{   
    ll size2 = s2.size(); ll size3 = s3.size();
    lpv(vert, s1)
    {   
        if (sz(adj[vert]) != (size2 + size3)) return -1;
        lpv(nbr, adj[vert])
        {
            if (s2.find(nbr) is s2.end() and s3.find(nbr) == s3.end()) 
                return -1;
        }
    }
    return 0;
...
int check3(set<int>& s1, set<int>& s2, set<int>& s3)
{
    if (check(s1, s2, s3) == -1) return -1;
    if (check(s2, s3, s1) == -1) return -1;
    if (check(s3, s1, s2) == -1) return -1;
    return 0;
}
...
{   
    ...
    if(s2.empty()){ cout << -1 << endl ; return 0 ;}

    vert = *s2.begin();
    lpv(nbr, adj[vert])
    {   
        if (s2.find(nbr) != s2.end())
        {
            s3.insert(nbr);
        }
        else
    ...
    if(s3.empty()){ cout << -1 << endl ; return 0 ; }
    // IF CORRECT, I SHOULD HAVE SET1 SET2 SET3 Complete

    if (check3(s1, s2, s3) == -1){ cout << -1 << endl ; return 0 ; }
    ...
    lpv(ind, ans) if (ind is 0) { cout << -1 << endl ; return 0; }

    lpv(ind, ans) cout << ind << " " ; cout << endl ; return 0;
}
